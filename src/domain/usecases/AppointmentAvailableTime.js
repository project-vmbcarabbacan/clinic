        ...
        try {
            ...
            while (currentTime.getHours() >= Constants.APPOINTMENT.TIMING_START && currentTime.getHours() < Constants.APPOINTMENT.TIMING_END) { // Until 5 PM
                ...
                if (existingAppointmentsCount < Constants.APPOINTMENT.SLOTS) { // If slot is not fully booked
                    availableSlots.push({
                        id: `available_time_${this.date.format_appointment_date(currentTime)}`,
                        title: currentTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
                        description: `Available slots: ${Constants.APPOINTMENT.SLOTS - existingAppointmentsCount}` // Remaining slots count
                    });
                }
